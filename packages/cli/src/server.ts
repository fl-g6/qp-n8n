...
import { InstanceSettings, JWT_AUTH_ACTIVE } from 'n8n-core';
...
import '@/controllers/quickplay.controller';
...
import { setupExternalJWTAuth } from './middlewares/external-jwt-auth';
...
export class Server extends AbstractServer {
	...
	setupQPJwt(publicApiEndpoint: string) {
		...
		const authIgnoreRegex = new RegExp(`^/(${nonUIRoutes.join('|')})/?.*$`);
		// Check for and validate JWT if configured
		// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
		const jwtAuthActive = process.env[JWT_AUTH_ACTIVE] ?? false;
		if (jwtAuthActive) {
	...
	async configure(): Promise<void> {
		...
		this.setupQPJwt(publicApiEndpoint);
